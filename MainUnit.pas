...
type
  TfrmMain = class(TForm)
    ...
    function  PutCoverToQueue(sFile, sCopies: String): Integer;
...
function FindFile(sFile: String): String;
var
  srchFiles: TSearchRec;
begin
Result := '';
if (FindFirst(sFile, faAnyFile, srchFiles) = 0) then
  Result := srchFiles.Name;
FindClose(srchFiles);
end;
...
function TfrmMain.PutCoverToQueue(sFile, sCopies: String): Integer;
var
  sOutFile: PChar;
  sCfg:     String;
begin
...
sOutFile := PChar(edCovers.Text + '2_na_list\' + sFile);
if (not CopyFile(PChar(sFile), sOutFile, True)) then
...
var
  ...
  iFound:     Integer;
begin
if FindFile(edOrders.Text) <> '' then
...
begin
  ...
  for i := 0 to sgOrder.RowCount - 1 do
    ...
    begin
      ...
      sPdfFile := FindFile(sDir + '*' + sIsbn + '*.*');
      if (sPdfFile <> '') then
      begin
        sDir := sDir + sPdfFile;
        if (sDir[Length(sDir)] <> '\') then
          sDir := sDir + '\';
        sPdfFile := FindFile(sDir + '*' + sIsbn + '-cover.*');
        if (sPdfFile <> '') then
        begin
...
//          PutCoverToQueue(sDir + sPdfFile, i);
        end
        else
        begin
